...
export const LanguageSelector = () => {
    ...
    useEffect(() => {
        const initialLocale = ['es', 'en', 'ca'].includes(navigator.language.split('-')[0]) 
            ...
            : 'en';
        mediaService.setLanguage(initialLocale);
    }, []);

    const handleLanguageChange = (langCode: string) => {
        changeLanguage(langCode);
        mediaService.setLanguage(langCode);
    };
    ...
    return (
        <div className="dropdown dropdown-end">
            ...
            <ul tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-1 w-36 p-2 shadow-sm">
                {languages.map((lang) => (
                    <li key={lang.code}>
                        <div className="btn btn-ghost flex justify-start gap-2" 
